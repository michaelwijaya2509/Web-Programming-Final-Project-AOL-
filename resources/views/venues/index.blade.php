        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-10 relative z-40">
                <form action="{{ route('venues.index') }}" method="GET" id="filterForm">
                    ...
                    <div
                        class="bg-white p-3 rounded-2xl shadow-lg border border-gray-100 flex flex-col lg:flex-row items-stretch lg:items-center gap-3 relative z-40">
                        ...
                        <div class="w-full lg:w-64 relative group z-40">
                            ...
                            <select name="type"
                                ...
                                <option value="badminton" {{ request('type') == 'badminton' ? 'selected' : '' }}>Badminton
                                </option>
                                <option value="padel" {{ request('type') == 'padel' ? 'selected' : '' }}>Padel</option>
                                <option value="pickleball" {{ request('type') == 'pickleball' ? 'selected' : '' }}>
                                    Pickleball</option>
                    ...
                    {{-- PRICE MODAL --}}
                    <div id="priceModal" class="fixed inset-0 z-[9999] hidden pointer-events-none"
                        aria-labelledby="modal-title" role="dialog" aria-modal="true">
                        <div class="fixed inset-0 bg-gray-900/60 backdrop-blur-sm transition-opacity pointer-events-auto"
                            onclick="toggleModal('priceModal')"></div>
                        ...
                        <div class="fixed inset-0 z-[10000] overflow-y-auto pointer-events-none">
                            <div
                                ...
                                <div
                                    class="relative transform overflow-hidden rounded-2xl bg-white text-left shadow-2xl transition-all sm:my-8 sm:w-full sm:max-w-md border border-gray-100 pointer-events-auto">

                                    {{-- MODAL HEADER --}}
                                    <div
                                        class="bg-gray-50 px-4 py-3 sm:px-6 flex justify-between items-center border-b border-gray-100">
                                        <h3 class="text-base font-bold leading-6 text-gray-900" id="modal-title">Filter
                                            Range Harga</h3>
                                        <button type="button" onclick="toggleModal('priceModal')"
            ...
            <div class="flex flex-col md:flex-row justify-between items-center mb-8 gap-4">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <div class="relative group">
                        <button
                            class="flex items-center gap-2 font-bold text-gray-800 bg-white border border-gray-200 px-4 py-2 rounded-lg hover:border-[#FF6700] transition">
                            Popularitas <i class="fas fa-chevron-down text-xs text-[#FF6700]"></i>
                        </button>

                        <div
                            class="absolute right-0 mt-2 w-48 bg-white rounded-lg shadow-xl border border-gray-100 hidden group-hover:block z-10">
                            <a href="#"
                                class="block px-4 py-2 text-sm text-gray-700 hover:bg-orange-50 hover:text-[#FF6700]">Harga
                                Termurah</a>
                            <a href="#"
                                class="block px-4 py-2 text-sm text-gray-700 hover:bg-orange-50 hover:text-[#FF6700]">Rating
                                Tertinggi</a>
                        </div>
                    </div>
                </div>
                    ...
                    @foreach ($venues as $venue)
                        <a href="{{ url('/venue/' . $venue->id) }}"
                            ...
                            <div class="relative h-56 bg-gray-200 overflow-hidden">
                                ...
                                <div
                                    class="absolute top-3 left-3 bg-white/90 backdrop-blur-sm px-3 py-1 rounded-full text-xs font-bold text-[#FF6700] shadow-sm">
                                    {{ $venue->type }}
                                </div>
                            </div>
                            ...
                            <div class="p-5">
                                <div class="flex justify-between items-start mb-2">
                                    ...
                                    <div
                                        class="flex items-center gap-1 text-xs font-bold text-gray-900 bg-gray-100 px-2 py-1 rounded">
                                        <i class="fas fa-star text-yellow-400"></i>

                                        {{-- Mengambil rata-rata dari relasi 'ratings' --}}
                                        {{ number_format($venue->ratings->avg('rating') ?? 0, 1) }}
                                    </div>
                                </div>

                                <h3
                                    class="text-lg font-bold text-gray-900 mb-1 group-hover:text-[#FF6700] transition line-clamp-1">
                                    {{ $venue->name }}
                                </h3>
                                ...
                                <div class="border-t border-gray-100 pt-4 flex items-center justify-between">
                                    ...
                                    <span
                                        class="text-sm font-semibold text-[#FF6700] bg-orange-50 px-3 py-2 rounded-lg group-hover:bg-[#FF6700] group-hover:text-white transition">
                                        Booking
                                    </span>
                                </div>
                            </div>
